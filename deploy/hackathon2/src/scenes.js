...
export const SCENES = [
  ...
  // #2/2 투 트랙 전용(2026-08-12 요청 — "배경도 투트랙을 설명하는 배경으로").
  // 코스가 두 갈래로 갈라져 하나는 관중이 모인 잔디밭에서 올라오고
  // (Bottom-up · 챌린저), 하나는 언덕 위 사옥에서 내려온다(Top-down ·
  // 마스터). 화면의 이야기가 카드 두 장이 하는 말과 같다.
  //
  // **가운데는 비어 있다.** 두 트랙 카드가 무대의 대부분을 덮으므로 씬이
  // 실제로 보이는 것은 위아래 띠와 좌우 엿보기 자리뿐이다 — 그 자리에
  // 갈라지는 두 레인과 사옥·관중이 오도록 프롬프트를 잡았다.
  //
  // 원본이 1376×768(1k)이라 1920으로 늘려 구웠다. 다른 씬(2400px)보다
  // 작지만, 이 장은 씬의 90%가 카드에 덮여 있어 실제로 보이는 것은
  // 초점이 흐린 하늘과 노면뿐이다. 크게 띄울 일이 생기면 4k로 다시 뽑는다.
