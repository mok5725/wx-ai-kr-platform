...
// 팀 슬라이드에서는 크루 명단을 함께 보낸다. 화면에는 이름만 한 줄로 뜨는데,
// 진행자는 팀장에게 마이크를 넘기기 전에 소속까지 보고 부르는 편이 낫다.
// slides.js에 명단을 복사해 두지 않고 content.js를 참조하므로 원본이 하나다.
function crewFor(slide) {
  if (!slide.team) return [];
  const team = TEAMS.find((t) => t.no === slide.team);
  if (!team) return [];
  return team.crew.flatMap((group) => group.members.map((name) => ({
    name,
  ...
  channel.onmessage = (event) => {
    ...
    if (s.crew.length) {
      els.crew.appendChild(Object.assign(document.createElement('h2'), {
        ...
        textContent: '크루 — 마이크를 넘기기 전에 부를 이름',
      }));
      for (const m of s.crew) {
        ...
        item.append(name, document.createTextNode(` · ${m.company}`));
