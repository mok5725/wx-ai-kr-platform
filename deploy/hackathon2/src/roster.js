...
import { WELCOME, SPONSORS, EXPEDITION, COACHES, TASKS, ORG_TIERS, PERKS, TEAMS, STAFF_TAGS, chipTone } from './content.js';
...
// ── 크루 전체 보기 (챕터 4의 첫 장) ─────────────────────────────────
//
// 여덟 팀이 한 화면에 선다. 팀명·아이디어명·크루원 이름 셋만 — 상세는
// 이어지는 팀 장에서 팀장이 말한다. 순서는 TEAMS의 가나다순 그대로다.
//
...
// 이름을 채우면 같은 명단을 두 번 읽히게 되고, 박스가 커져 씬 가운데의
// 스타트라인 인파를 가린다.
// **팀명은 무조건 한 줄이다.** 여덟 칸이 같은 높이로 서야 격자가 표처럼
...
function renderTeamsAll(root) {
  ...
  TEAMS.forEach((team) => {
    ...
    ideaLines(team.idea).forEach((line, n) => {
      if (n) idea.appendChild(document.createElement('br'));
...
// ── 운영진 이름표 (챕터 5) ──────────────────────────────────────────
//
// 인물은 씬 그림에 이미 그려져 있다. 여기서는 그 아래에 이름표만 세운다 —
// x는 그림 속 인물의 가로 중심이고, 씬을 다시 뽑으면 content.js의
// STAFF_TAGS를 함께 고쳐야 한다.
//
// 이 이름표가 자리를 지키려면 **그 씬의 카메라가 정지해 있어야 한다**
// (scenes.js의 still). 씬이 확대되는데 이름표만 가만히 있으면 사람에게서
// 떨어져 나간다.
function renderStaffTags(root) {
  root.className = 'staff-tags';
  for (const p of STAFF_TAGS) {
    const tag = el('div', p.tight ? 'staff-tag staff-tag--tight' : 'staff-tag');
    tag.style.setProperty('--x', `${p.x}%`);

    const name = el('p', 'staff-tag__name');
    name.append(document.createTextNode(`${p.name} `), el('span', null, p.title));

    // 배역은 글자로 적지 않는다. 신호탄·스톱워치·메가폰처럼 캐릭터가 든
    // 소품이 이미 그 역할을 말하고 있고, 같은 말을 글로 또 적으면 그림이
    // 설명서가 된다.
    tag.append(el('p', 'staff-tag__nick', p.nickname), name);
...
const RENDERERS = {
  ...
  'staff-tags': renderStaffTags,
  'teams-all': renderTeamsAll,
